...
def check_claim_rejection(claim_reason, general_exclusion_list, prompt_template, threshold=0.4):
    ...
    for disease in general_exclusion_list:
        ...
        if similarity > threshold:
            ...
            new_prompt_template = """You are an AI assistant for verifying health insurance claims. You are given with the references for approving the claim and the patient details. Analyse the given data and give a good rejection. You the following guidelines for your analysis.
            PATIENT INFO : {patient_info}
            DOCUMENTS FOR CLAIM APPROVAL: {claim_approval_context}
...
def msg():
    ...
    updated_prompt = check_claim_rejection(claim_reason, general_exclusion_list, PROMPT)
    print(updated_prompt)
    
    prompt = PromptTemplate(input_variables=["claim_approval_context","patient_info"], template=updated_prompt)
    
    #     prompt = PromptTemplate(input_variables=["claim_approval_context","patient_info"], template=PROMPT)
